...
def paint_content(element: AnyElement, content: Content, rect: Rect) -> Paint:
    ...
    if isinstance(content.color, Color):
        ...
        z = element.style.layout.z
        return {p: P.blank(color=colors[p], z=z) for p, c in colors.items()}
...
def svg(paint: Paint) -> ElementTree:
    ...
    for y, cells in sorted(rows.items()):
        ...
        # Assumes resolved colors
        for bg_color, x_cells_group in groupby(cells, key=lambda x_cell: x_cell[1].style.background.hex):  # type: ignore[union-attr]
            ...
            for x, cell in x_cells:
                ...
                if cell.style.foreground != Color.from_name("white"):
                    # Assumes resolved colors
                    ts.attrib["fill"] = cell.style.foreground.hex  # type: ignore[union-attr]
                ts.text = cell.char
